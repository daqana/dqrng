...
In order to use these header files, you have to use at least C++11 link to the `BH` and `sitmo` packages as well.
This happens automatically when `Rcpp::sourceCpp` or `Rcpp::cppFunction` are used.
...
public:
  ...
  void seed(result_type seed, result_type stream) {state = seed;}
